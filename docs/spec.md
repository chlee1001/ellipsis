...
- While a pin is up, the icon brings the bar back on the first click and hides the set, the pins and the bar on the second. The rehide conditions end the pins with the set; the timeout restarts on every pin, and a menu open from a pinned item keeps the set shown, as before.
