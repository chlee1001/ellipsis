...
8. `scripts/run.sh`: bundle, copy to `/Applications`, launch. A debug build is `EllipsisDev.app` with the identifier `com.chlee1001.EllipsisDev`, so it runs next to a release build with its own settings. `MenuBarAgent` only matches apps in `/Applications` (see `docs/phase0.md`).
...
7. Settings — done. Tests write them with `defaults write com.chlee1001.EllipsisDev` before the launch, not through the Settings window; each test launches the app with the settings it needs. `AppState` does not see a `defaults write` while the app runs, so row 5a (a timeout change while shown) stays manual.
