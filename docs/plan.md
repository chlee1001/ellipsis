...
6. Pins — done, replacing the first click-through. `PinPolicy` holds the arithmetic (a click pins, a click on a pinned app unpins it, at most three, a click past the limit drops the oldest) with unit tests. `barClicked` pins, closes the bar and calls `RehideMonitor.rearm()`, so the timeout restarts; `applyCurrentState` subtracts the pins from what the restriction hides. Nothing is pressed on the user's behalf any more: the press raced the `MenuBarAgent` layout and lost the item its menu. With the permission, `checkPinsFit` takes a snapshot of the old layout first, waits for the newest assertion to report back (`waitUntilActivated`), then reads the layout settled against it (`MenuBarLayout.readSettled(after:)`), since reads taken before the restriction lands agree with each other and would miss the newest pin in the old layout and escalate for nothing. Escalation hides every other regular or accessory app while a pinned item is not drawn; a list that also names background processes is ignored by `MenuBarAgent` as a whole. `Logger` category `pins` in subsystem `au.ronny.Ellipsis` records each decision.
